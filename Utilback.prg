/*
 ...
 UtilBack - Utilities Backup
      ...
      Last change:  TG   16 Jan 2011    6:38 pm
*/

#include "winrent.ch"
#define BACKUPFILE "WinRentBackup"
...
local cBakStr := Oddvars( SYSPATH ) + "zip " + Oddvars( SYSPATH ) + BACKUPFILE + " " +;
                 Oddvars( SYSPATH ) + "*.dbf " +;
                 Oddvars( SYSPATH ) + "iComment\*.* " +;
                 Oddvars( SYSPATH ) + "dComment\*.* " +;
                 Oddvars( SYSPATH ) + "mComment\*.*"
...
while TRUE
 ...
 aadd( aArray, { 'Backup', 'Backup Data files to External Storage' } )
 aadd( aArray, { 'Restore', 'Restore file from External Storage' } )
 ...
 case choice = 2
  ...
  if !NetUse( "master", EXCLUSIVE, 10 )
   ...
   if len(mdir) > 0
    Highlight( 05, 12, 'Minimum Space Required ',transform(mdir[1,2],'999,999,999,999')+' Bytes')
    Highlight( 07, 12, 'The date of the current zipped file is',dtoc(mdir[1,3]))
